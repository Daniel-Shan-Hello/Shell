...
/*
 * print_signal_mask - Prints signals that are blocked in a signal mask.
 * Not async-signal-safe
 */
void print_signal_mask(const sigset_t *mask) {
    bool is_empty = true;
    for (int sig = 1; sig < NSIG; ++sig) {
        int is_set = sigismember(mask, sig);
        if (is_set == 1) {
            const char *name = strsignal(sig);
            if (name == NULL) name = "Unknown";
            printf("Blocked: %s (signal %d)\n", name, sig);
            is_empty = false;
        }
        else if (is_set == -1) {
            // sig number out of range or other error
            perror("sigismember");
            break;
        }
    }
